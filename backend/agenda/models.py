...
class Agenda(models.Model):
    ...
    def __str__(self):
        return f"Agenda {self.id} - {self.dia} | Médico: {self.medico}"
...
class Horario(models.Model):
    ...
    def __str__(self):
        return f"Horario: {self.hora} - ({self.agenda.dia} - Médico: {self.agenda.medico.nome} | CRM: {self.agenda.medico.crm})"